...
export class Space {
	...
	/**
	 * @description draw space objects
	...
	/**
	 * @description load starter scene
	 */
	loadOriginScene () {
	...
	/**
	 * @description add new space obj on the scene
	...
	loadNewSpaceObj (e) {
		...
		const z = 0;
		const vx = (e.clientX-this.mousePressX)/35;
		const vy = (e.clientY-this.mousePressY)/35;
		const vz=0;
	 ...
	 */
	settings () {
		document.querySelector('#reset-btn').addEventListener('click', this.loadOriginScene.bind(this));
		...
		this.canvas.addEventListener('mouseup', this.loadNewSpaceObj.bind(this));
	}
}